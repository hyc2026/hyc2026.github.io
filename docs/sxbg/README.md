...
授信报告往往文本篇幅长，内容过于繁杂，靠人类逐字阅读来获取信息效率低，因此从授信报告中提取结构化的信息往往能节省大量的人力物力成本。
...
上述关系和描述共同抽取的方法经试验发现效果不理想，因此采用先抽关系再抽描述的方法，通过`[CLS]`、`[S-type]`和`[O-type]`三个位置的输出进行拼接，输入分类器，共同预测关系的种类。
...
在关系抽取的基础上，将每个分隔符替换成`[CLS]`，用于分类该子句是否为`S`-主体描述、`P`-关系描述、`O`-客体描述和`X`-非描述。
...
{
    "id": 1,
    "tokens": ["[CLS]", "[O-ORG]", "是", "一", "所", "电", "子", "商", "务", "公", "司", "[CLS]", "其", "股", "东", "之", "一", "[S-PER]", "曾", "经", "是", "一", "名", "英", "语", "教", "师", "[CLS]", "占", "股", "约", "7", ".", "6", "%", "[CLS]"],
    "cls_list": [0, 11, 27, 35],
    "desc": ["X", "O", "S", "P"]
}
```
